...
/////log
global.ownernumber = '923000321640' 
//Gallery/database
...
//time
const replay = (teks) => {
            Riaz-Hussain.sendMessage(m.chat, { text: teks}, { quoted: m})
        }
const xtime = moment.tz('Asia/Kolkata').format('HH:mm:ss')
        const xdate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
        const time2 = moment().tz('Asia/Kolkata').format('HH:mm:ss')  
         if(time2 < "23:59:00"){
var Saifitimewisher = `Good Night 🌌`
 }
 if(time2 < "19:00:00"){
var Saifitimewisher = `Good Evening 🌃`
 }
 if(time2 < "18:00:00"){
var Saifitimewisher = `Good Evening 🌃`
 }
 if(time2 < "15:00:00"){
var Saifitimewisher = `Good Afternoon 🌅`
 }
 if(time2 < "11:00:00"){
var Saifitimewisher = `Good Morning 🌄`
 }
 if(time2 < "05:00:00"){
var Saifitimewisher = `Good Morning 🌄`
 } 
module.exports = Saifi = async (Saifi, m, msg, chatUpdate, store) => {
    try {
        const {
            type,
            quotedMsg,
        ...
        const botNumber = await Saifi.decodeJid(Saifi.user.id)
        ...
        const groupMetadata = m.isGroup ? await Saifi.groupMetadata(m.chat).catch(e => {}) : ''
...
let { key } = await Riaz-Hussain.sendMessage(from, {text: 'ʟᴏᴀᴅɪɴɢ...'})

for (let i = 0; i < Ayushlod.length; i++) {
await Saifi.sendMessage(from, {text: Ayushlod[i], edit: key });
}
}
        ...
        if (autoread) {
            Saifi.readMessages([m.key])
        }
        if (global.autoTyping) {

        Saifi.sendPresenceUpdate('composing', from)
        ...
        Saifi.sendPresenceUpdate('recording', from)

        }

        
        //bot number online status, available=online, unavailable=offline
        Saifi.sendPresenceUpdate('unavailable', from)
        
        if (global.autorecordtype) {
        let RiazHussainrecordin = ['recording','composing']

        let RiazHussainrecordinfinal = Riaz-Hussainrecordin[Math.floor(Math.random() * Riaz-Hussainrecordin.length)]

        Saifi.sendPresenceUpdate(Riaz-Hussainrecordinfinal, from)

        }
        
        if (autobio) {
            Saifi.updateProfileStatus(`Hey, future leaders! 🌟 Saifi-Md is here to inspire and lead, thanks to Riaz-Hussain Botz, Inc. 🚀 ${runtime(process.uptime())} `).catch(_ => _)
        }
        if (m.sender.startsWith('212') && global.anti212 === true) {
            return Saifi.updateBlockStatus(m.sender, 'block')
        }

        let list = []
        for (let i of owner) {
list.push({
	    	displayName: await Saifi.getName(i),
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Saifi.getName(i)}\nFN:${await Saifi.getName(i)}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${ytname}\nitem2.X-ABLabel:YouTube\nitem3.URL:${socialm}\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	...
	//chat counter (console log)
        ...
        if (m.isGroup && !m.key.fromMe) {
            ...
            if (afk.checkAfkUser(m.sender, _afk)) {
                ...
                Saifi.sendTextWithMentions(m.chat, `@${m.sender.split('@')[0]} have returned from afk`, m)
            }
        }
...
if (isCreator) return m.reply(bvl)
        await Saifi.sendMessage(m.chat,
			    {
			        delete: {
			            remoteJid: m.chat,
			            fromMe: false,
			...
			Saifi.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
Saifi.sendMessage(from, {text:`\`\`\`「 Link Detected 」\`\`\`\n\n@${m.sender.split("@")[0]} Has been kicked because of sending link in this group`, contextInfo:{mentionedJid:[m.sender]}}, {quoted:m})
} else {
}
...
if (smallinput.includes(`afternoon`)) {
        ...
        if (smallinput.includes(`Night`)) {
                  ...
                  reply(`i am Saifi BOT, a whatsapp bot created by Riaz-Hussain, maybe am not the one whom u asked but i just want u to knw that i am a bot, type #menu to see all available features😊`)

                  }

       if (smallinput.includes(`Saifi`)) {

         reply(`Hello frnd mai Saifi BOT hun😁.\ntype😊👉${prefix}menu`)
       ...
       if (smallinput.includes(`@923000321640`)) {
       ...
       if (smallinput.includes(`@923000321640`)) {
                  ...
                  reply(`sorry mate, Riaz-Hussain is busy`)
...
var groupe = await Saifi.groupMetadata(from)
var members = groupe['participants']
var mems = []
members.map(async adm => {
mems.push(adm.id.replace('c.us', 's.whatsapp.net'))
})
Saifi.sendMessage(from, {text: `\`\`\`「 ⚠️Warning⚠️ 」\`\`\`\n\nIf you're not an admin, don't send any link in this group or u will be kicked immediately!`, contextInfo: { mentionedJid : mems }}, {quoted:m})
} else if (args[0] === "off") {
...
var medis = await Saifi.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (args[0] == `full`) {
...
var memeg = await Saifi.updateProfilePicture(botNumber, { url: medis })
fs.unlinkSync(medis)
...
break
            ...
            case 'join':
                try {
                    ...
                    await Saifi.groupAcceptInvite(result).then((res) => reply(json(res))).catch((err) => reply(json(err)))
                } catch {
            ...
            case 'setppbot':
                ...
                if (args[0] == 'full') {
                    ...
                    var memeg = await Saifi.updateProfilePicture(botNumber, {
                        url: medis
                    })
                    fs.unlinkSync(medis)
                    reply(mess.done)
            ...
            case 'block':
                ...
                await Saifi.updateBlockStatus(blockw, 'block').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'unblock':
                if (!isCreator) return reply(mess.owner)
                let blockww = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
                await Saifi.updateBlockStatus(blockww, 'unblock').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'leave':
                if (!isCreator) return reply(mess.owner)
                if (!m.isGroup) return reply(mess.group)
                reply('🟨Bye Everyone 🥺')
                await Saifi.groupLeave(m.chat)
                break
            case 'bcgc':
            case 'bcgroup': {
                if (!isCreator) return reply(mess.owner)
                if (!text) return reply(`Which text?\n\nExample : ${prefix + command} It's holiday tomorrow `)
                let getGroups = await Saifi.groupFetchAllParticipating()
            ...
            case 'getcase':
                if (!isCreator) return reply(mess.owner)
                const getCase = (cases) => {
                    return "case" + `'${cases}'` + fs.readFileSync("Riaz-HussainBug3.js").toString().split('case \'' + cases + '\'')[1].split("break")[0] + "break"
                }
                reply(`${getCase(q)}`)
                break
            case 'delete':
            ...
            case 'closetime':
                ...
                setTimeout(() => {
                    ...
                    Saifi.groupSettingUpdate(m.chat, 'announcement')
                    reply(close)
                }, timer)
                break
            case 'opentime':
                ...
                setTimeout(() => {
                    ...
                    Saifi.groupSettingUpdate(m.chat, 'not_announcement')
                    reply(open)
                }, timer)
                break
            case 'kick':
                ...
                await Saifi.groupParticipantsUpdate(m.chat, [blockwww], 'remove').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'add':
                if (!m.isGroup) return reply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin)
                ...
                await Saifi.groupParticipantsUpdate(m.chat, [blockwwww], 'add').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'promote':
                if (!m.isGroup) return reply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin)
                ...
                await Saifi.groupParticipantsUpdate(m.chat, [blockwwwww], 'promote').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'demote':
                if (!m.isGroup) return reply(mess.group)
                if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin)
                ...
                await Saifi.groupParticipantsUpdate(m.chat, [blockwwwwwa], 'demote').then((res) => reply(json(res))).catch((err) => reply(json(err)))
                break
            case 'setname':
            case 'setsubject':
                if (!m.isGroup) return reply(mess.group)
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (!text) return 'Text ?'
                await Saifi.groupUpdateSubject(m.chat, text).then((res) => reply(mess.done)).catch((err) => reply(json(err)))
                break
            case 'setdesc':
            case 'setdesk':
                if (!m.isGroup) return reply(mess.group)
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (!text) return 'Text ?'
                await Saifi.groupUpdateDescription(m.chat, text).then((res) => reply(mess.done)).catch((err) => reply(json(err)))
                break
            case 'setppgroup':
            case 'setppgrup':
            case 'setppgc':
                ...
                if (args[0] == 'full') {
                    ...
                    var memeg = await Saifi.updateProfilePicture(m.chat, {
                        url: medis
                    })
                    fs.unlinkSync(medis)
                    reply(mess.done)
 ...
 *Message : ${args.join(" ") ? args.join(" ") : 'no message'}*\n\n`
 ...
 Saifi.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: m })
 }
 break
            ...
            case 'grup':
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (args[0] === 'close') {
                    await Saifi.groupSettingUpdate(m.chat, 'announcement').then((res) => reply(`*_SUCCESSFULLY CLOSED THE GRUP_\n\ngrup has been closed for the time being 😽*`)).catch((err) => reply(json(err)))
                } else if (args[0] === 'open') {
                    await Saifi.groupSettingUpdate(m.chat, 'not_announcement').then((res) => reply(`*THE GROUP HAS BEEN OPENED SUCCESSFULLY*`)).catch((err) => reply(json(err)))
                } else {
            ...
            case 'editinfo':
                ...
                if (!isBotAdmins) return reply(mess.botAdmin)
                if (args[0] === 'open') {
                    await Saifi.groupSettingUpdate(m.chat, 'unlocked').then((res) => reply(`Successfully Opened Group Edit Info 🕊️`)).catch((err) => reply(json(err)))
                } else if (args[0] === 'close') {
                    await Saifi.groupSettingUpdate(m.chat, 'locked').then((res) => reply(`Successfully Closed Group Edit Info🕊️`)).catch((err) => reply(json(err)))
                } else {
      ...
      case "grouplink":
        {
          ...
          let response = await Saifi.groupInviteCode(m.chat);
          Saifi.sendText(
            m.sender,
        ...
        await Saifi.sendMessage(m.chat, { video: { url: `https://media.tenor.com/hzWYhzhMTeEAAAPo/Saifi-useless.mp4` }, caption: 'I sent you the Group Link in personal message.\n Pls check.', gifPlayback: true }, { quoted: m });
        break
            ...
            case 'resetlink':
                ...
                if (!isAdmins && !isGroupOwner && !isCreator) return reply(mess.admin)
                if (!isBotAdmins) return reply(mess.botAdmin)
                await Saifi.groupRevokeInvite(m.chat)
                    .then(res => {
...
const axios = require("axios");
let repoInfo = await axios.get("https://api.github.com/repos/Riaz-Hussain-PANDEY023/Saifi-MD");
...
©️ *Riaz-Hussain Bots inc* 
*❝ Dont forget to give a Star ⭐ to the repo.*`

        Saifi.sendMessage(from, { video: { url: 'https://media.tenor.com/Zco-fadJri4AAAPo/code-matrix.mp4' }, gifPlayback: true, caption: scritxt }, { quoted: m })
}
        break;
...
case 'owner': {
const repf = await Saifi.sendMessage(from, { 
contacts: { 
...
break
            ...
            case 's': {
                ...
                if (/image/.test(mime)) {
                    ...
                    let encmedia = await Saifi.sendImageAsSticker(m.chat, media, m, {
                    ...
                    let media = await quoted.download()
                    let encmedia = await Saifi.sendVideoAsSticker(m.chat, media, m, {
            ...
            case 'smeme': {
                ...
                let dwnld = await Saifi.downloadAndSaveMediaMessage(qmsg)
                let fatGans = await TelegraPh(dwnld)
                let smeme = `https://api.memegen.link/images/custom/${encodeURIComponent(bawah)}/${encodeURIComponent(atas)}.png?background=${fatGans}`
                let pop = await Saifi.sendImageAsSticker(m.chat, smeme, m, {
                    packname: global.stickername,
...
Saifi.downloadAndSaveMediaMessage(quoted, "gifee")
Saifi.sendMessage(from, {sticker:fs.readFileSync("gifee.webp")},{quoted:m})
} else if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await Saifi.sendImageAsSticker(m.chat, media, m, { packname: pcknm, author: atnm })
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return reply('Maximum 10 Seconds!')
let media = await quoted.download()
let encmedia = await Saifi.sendVideoAsSticker(m.chat, media, m, { packname: pcknm, author: atnm })
} else {
reply(`Photo/Video?`)
}
}
break
            ...
            case 'toimg': {
                ...
                reply(mess.wait)
                let media = await Saifi.downloadAndSaveMediaMessage(qmsg)
            ...
            break
            case 'checkdeath':
             if (!text) return replay(`Use Someone's Name, Example : ${prefix + command} Riaz-Hussain`)
            ...
            case 'tomp3': {
                ...
                reply(mess.wait)
                let media = await Saifi.downloadMediaMessage(qmsg)
                let audio = await toAudio(media, 'mp4')
                Saifi.sendMessage(m.chat, {
                    document: audio,
                    mimetype: 'audio/mp3',
                    fileName: `Saifi-bot.mp3`
                }, {
                    quoted: m
                })
            ...
            case 'emojimix': {
                ...
                for (let res of anu.results) {
                    let encmedia = await Saifi.sendImageAsSticker(m.chat, res.url, m, {
                        packname: global.stickername,
            ...
            case 'toviewonce': {
                ...
                if (/image/.test(mime)) {
                    anuan = await Saifi.downloadAndSaveMediaMessage(quoted)
                    Saifi.sendMessage(m.chat, {
                        image: {
                            url: anuan
                        },
                    ...
                    anuanuan = await Saifi.downloadAndSaveMediaMessage(quoted)
                    Saifi.sendMessage(m.chat, {
                        video: {
                            url: anuanuan
                        },
...
let ceknye = await Saifi.onWhatsApp(bnnd)
...
case 'delowner':
                if (!isCreator) return reply(mess.owner)
if (!args[0]) return reply(`Use ${prefix+command} nomor\nExample ${prefix+command} 923000321640`)
...
break
      ...
      case 'qc': {
                ...
                let ppnyauser = await await Saifi.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/6880771a42bad09dd6087.jpg')
                const rest = await quote(q, pushname, ppnyauser)
                reply(mess.wait)
                Saifi.sendImageAsSticker(m.chat, rest.result, m, {
                    packname: `${global.stickername}`
...
case 'play':  case 'song': {
Saifi.sendMessage(from, { react: { text: "📥", key: m.key }}) 
if (!text) return reply(`Example : ${prefix + command} anime whatsapp status`)
const RiazHussainplaymp3 = require('./Gallery/lib/ytdl2')
let yts = require("youtube-yts")
        let search = await yts(text)
        let anup3k = search.videos[0]
const pl= await Riaz-Hussainplaymp3.mp3(anup3k.url);
...
const RiazHussainvidoh = require('./Gallery/lib/ytdl2')
if (args.length < 1 || !isUrl(text) || !RiazHussainvidoh.isYTUrl(text)) reply(`Where is the link??\n\nExample : ${prefix + command} https://youtube.com/watch?v=PtFMh6Tccag%27 128kbps`)
const vid=await RiazHussainvidoh.mp4(text)
const ytc=`
...
Saifi.sendMessage(from, { react: { text: "🤖", key: m.key }}) 
              ...
              try {
                ...
                if (response1.status === 200 && responseData1 && responseData1.status === true && responseData1.data) {
                  ...
                  const me = m.sender;
                  await Saifi.sendMessage(m.chat, { text: message, mentions: [me] }, { quoted: m });
                } else {
        ...
        try {
          await Saifi.sendMessage(m.chat, { image: { url: apiUrl } }, { quoted: m });
        } catch (error) {
          console.error(error);
        ...
        const helptxt = `_*📍[Rules for Saifi Md usage]📍*_\n\n\n*>>>* use -support to get the Official group link in your dm.\n\n*--->* If you want to add Saifi-Md in your group the contact the owner by *-owner/-mods* \n\n*--->* Dont use wrong command, use the command given in the *-help* list \n\n* Dont spam the bot with commands if Saifi-Md is not responding, its means the maybe owner is offline or facing internet issue. \n\n*IF YOU DONT FOLLOW THE RULES THEN YOU WILL BE BANNED* 🚫 \n\n\n*©️ RiazHussain Bots inc* `

        Saifi.sendMessage(from, { video: { url: 'https://c.tenor.com/geMdtLCXZkAAAAPo/rules.mp4' }, gifPlayback: true, caption: helptxt }, { quoted: m })

        break
      case 'hii': case 'hi': case 'Hi':
       
        
        let txxt = `👋🏻 Hi *${pushname}*, i am  *Saifi-Md*📍\nA whatsapp bot created by: RiazHussain \n\n I don't have time for chit-chat Darling. Use command from *${prefix}help* list if you want me to do anything.`

        Saifi.sendMessage(m.chat, { image: { url: "./Gallery/ch1.jpg" }, caption: txxt, gifPlayback: true }, { quoted: m });
        break
      case "support":
...
https://chat.whatsapp.com/Jllsl2OaQNoBjepxzuVsZM`
        ...
        await Saifi.sendMessage(m.sender,{ image: {url: "./Gallery/sup.jpg"}, caption: `${tex}` },);

        await Saifi.sendMessage(m.chat, { image: { url: "./Gallery/ch2.jpg" }, caption: 'I sent you the support Link in personal message.\n Pls check.', gifPlayback: true }, { quoted: m });
        break

      case "info":
            Saifi.sendMessage(from, { react: { text: "ℹ️", key: m.key }}) 
        let ifx = `🌟『𝕄𝕒𝕣𝕚𝕒-𝕄𝕕 』🌟
*🌟Description:* A WhatsApp Bot With Rich  features based on Saifi
*🚦Uptime:* ${runtime(process.uptime())}
*🕸Version:* 1.2.0
*👤Creator:*  𝑨𝒚𝒖𝒔𝒉 𝒑𝒂𝒏𝒅𝒆𝒚\n
*Powered by RiazHussain*`
Saifi.sendMessage(m.chat, { image: { url: "./Gallery/ch3.jpg" }, caption: ifx, gifPlayback: true }, { quoted: m });
        break

 
      case 'owner': case 'creator': case 'mod': case 'mods': {
        Saifi.sendContact(m.chat, global.Owner, m)
      }
...
var inputnumber = text.split(" ")[0]
        ...
        for (let i = 0; i < randomxx; i++) {
            ...
            var anu = await Saifi.onWhatsApp(`${number0}${i}${number1}@s.whatsapp.net`)
            var anuu = anu.length !== 0 ? anu : false
            try {
                try {
                    var anu1 = await Saifi.fetchStatus(anu[0].jid)
                } catch {
                    var anu1 = '401'
                }
...
"eat 2 tablespoons of rice without any side dishes, if it's dragging you can drink",
         ...
         "say i love the bot owner Saifi through voice note",
         ...
         "breakup with your best friend for 5hrs without telling him/her that its a dare",
          ...
          "send abusive words in any grup, excepting this grup, and send screenshot proof here"
                   ...
                   const Saifidareww = dare[Math.floor(Math.random() * dare.length)]
                   buffer = await getBuffer(`https://graph.org/file/8dd92e67cd4019b092f53.jpg`)
                   Saifi.sendMessage(from, { image: buffer, caption: '*You have chosen Dare*\n\n'+ Saifidareww }, {quoted:m})
                   break
...
case 'truth':
                 ...
                 "do you love the bot creator RiazHussain?",
                 ...
                 "do you play pubg, if you then send ur id number"
                           ...
                           const Saifitruthww = truth[Math.floor(Math.random() * truth.length)]
                           buffer = await getBuffer(`https://graph.org/file/8dd92e67cd4019b092f53.jpg`)
                           Saifi.sendMessage(from, { image: buffer, caption: '*You have chosen Truth*\n'+ Saifitruthww }, {quoted:m})
                           break
                           
  case 'menu': case 'help': case 'h': 
  const txt = `Konichiwa *${pushname}* Senpai! 🎄 Merry Christmas! ,

I am *Saifi-Md*, a bot developed by *RiazHussain*.
...
🍂 Obtain the full list of NSFW commands by typing  *${prefix}nsfwmenu*`
  ...
  if (randomImage) {
    Saifi.sendMessage(from, { image: { url: randomImage }, caption: txt }, { quoted: m });
  }
  ...
  break; 
       ...
       case 'circlevideo': {
try {
const Saifibaileys = await require("@whiskeysockets/baileys").generateWAMessageContent({ video: await m.quoted.download() }, { upload: Saifi.waUploadToServer })
await Saifi.relayMessage(from, { ptvMessage: { ...Saifibaileys.videoMessage } }, {})
} catch (err) {
...
└──────────⊰ 
`
        Saifi.sendMessage(m.chat, { image: { url: "./Gallery/nsfw.jpg" }, caption: nsfwmenu }, { quoted: m });
        break
              case 'nsfw': {
   Saifi.sendMessage(from, { react: { text: "🔞", key: m.key }}) 
 if (!m.isGroup) return m.reply(mess.group);
...
Saifi.sendMessage(from, {text: `\`\`\``, contextInfo: { mentionedJid : mems }}, {quoted:m})
} else if (args[0] === "off") {
...
var Saifiyresult = pickRandom(ahegaonsfw)
Saifi.sendMessage(m.chat, { caption: mess.done, image: { url: Saifiyresult.url } }, { quoted: m })
break
...
Saifi.sendMessage(m.chat, { caption: mess.done, image: { url:waifudd.data.url } }, { quoted: m })
break
...
break;
			...
			Saifi.sendMessage(m.chat, { image: proses, caption: mess.done}, { quoted: m})
			}
			break
              case 'awesomecheck':
  case 'greatcheck':
...
if (mentionByReply) {
Saifi.sendMessage(from, { text: 'Question : *' + cex + '*\nChecker : ' + `@${mentionByReply.split('@')[0]}` + '\nAnswer : ' + cek2 + '%', mentions: [mentionByReply] }, { quoted: m })
...
case 'hidetag': {  
           if (!m.isGroup) return reply(mess.group)
                if (!isAdmins && !isCreator) return reply(mess.admin)
                if (!isBotAdmins) return reply(mess.botAdmin)
 Saifi.sendMessage(m.chat, { text : args.join(" ") ? args.join(" ") : '' , mentions: participants.map(a => a.id)}, { quoted: m })
 }
 break
 ...
 *Message : ${text}*\n\n`
 ...
 Saifi.sendMessage(m.chat, { text: teks, mentions: groupAdmins}, { quoted: m })
 }
 break
...
This Code is made with ❤️ by Shizo The Techie for Saifi-MD (Ayush) 
...
break
      ...
      if (!args.join(" ")) return reply(`${pushname} Please provide a search term!`);
        ...
        for (let i = 0; i < results.length; i++) {
          Saifi.sendMessage(m.chat, { image: { url: results[i] } }, { quoted: m });
        }
      }
        break;  
...
Saifi.sendMessage(from, { react: { text: "🔖", key: m.key }}) 
        ...
        Saifi.sendText(ownernumber + '@s.whatsapp.net', util.format(err), m)
